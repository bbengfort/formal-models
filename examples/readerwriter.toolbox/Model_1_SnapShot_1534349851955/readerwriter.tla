...
(*--algorithm reader 
variables 
  queue = <<>>;       \* message queue that writers writes to and reader reads from 
  dropped_count = 0;  \* number of messages that are dropped
  ...
  HandleValue: 
    ...
    or 
      ...
      ReadFailure: 
        ...
        \* without this guard, then deadlock can occur (which is worse?) 
        if Len(queue) < MaxQueueSize then 
          write_to_queue(self, val, FALSE);
        else
          \* this can cause an unbounded model, where drop just increases, so we use a state 
          \* constraint on the dropped_count to limit the number of dropped message states 
...
\* BEGIN TRANSLATION
\* Process variable val of process reader at line 69 col 3 changed to val_
VARIABLES queue, dropped_count, pc, stack
...
vars == << queue, dropped_count, pc, stack, val, failed_message, to_write, 
           val_ >>
...
HandleValue(self) == /\ pc[self] = "HandleValue"
                     ...
                     /\ UNCHANGED << queue, dropped_count, to_write, val_ >>
...
Write == /\ pc["writer"] = "Write"
         ...
         /\ UNCHANGED << dropped_count, stack, val, failed_message, to_write, 
                         val_ >>
              ...
              /\ IF Len(queue) > 0
                    ...
                    ELSE /\ pc' = [pc EXCEPT ![self] = "EndRead"]
                         /\ UNCHANGED << queue, stack, val, failed_message, 
                                         val_ >>
              /\ UNCHANGED << dropped_count, to_write >>

EndRead(self) == /\ pc[self] = "EndRead"
                 /\ pc' = [pc EXCEPT ![self] = "Read"]
                 /\ UNCHANGED << queue, dropped_count, stack, val, 
                                 failed_message, to_write, val_ >>
...
\* Last modified Wed Aug 15 12:17:19 EDT 2018 by benjamin
