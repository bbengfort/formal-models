      ...
      queue := Tail(queue);
       ...
       HandleValue: 
        ...
        or 
          ...
          write_to_queue([writer |-> "reader", msg |-> "failed", val |-> val]); 
          val := "none"; 
        end either;       
    end if; 
  EndRead:
...
HandleValue == /\ pc["reader"] = "HandleValue"
               /\ \/ /\ TRUE
                     /\ UNCHANGED <<queue, val>>
                  \/ /\ Len(queue) < MaxQueueSize
                     /\ queue' = Append(queue, ([writer |-> "reader", msg |-> "failed", val |-> val]))
                     /\ val' = "none"
...
\* Last modified Wed Aug 15 11:34:03 EDT 2018 by benjamin
