...
variables 
  flags = [t \in 1..Cardinality(Threads) |-> FALSE];
  next = 1;
...
P2: 
  ...
  \* we enter the wire loop if anyone but us is True  
  while \A f \in {t \in Threads: t # self}: flags[f]   do 
    P2_1: 
      if next # self then 
...
P3: 
  \* next is a tie-breaker, round-robin the tie-break 
  if next = Cardinality(Threads) then 
    next := 1; 
  else 
    next := next + 1; 
  end if;
...
P2(self) == /\ pc[self] = "P2"
            /\ IF \A f \in {t \in Threads: t # self}: flags[f]
...
\* Last modified Wed Aug 15 17:31:16 EDT 2018 by benjamin
