...
Upgrade(self) == /\ pc[self] = "Upgrade"
                 ...
                 /\ pc' = [pc EXCEPT ![self] = "ReturnFirst"]
                 /\ UNCHANGED << load_balancer, first >>

ReturnFirst(self) == /\ pc[self] = "ReturnFirst"
                     /\ status[first] = "ready"
                     /\ load_balancer' = (load_balancer \union {first})
                     /\ pc' = [pc EXCEPT ![self] = "Done"]
                     /\ UNCHANGED << version, status, first >>

server(self) == Upgrade(self) \/ ReturnFirst(self)
...
\* Last modified Thu Aug 16 14:00:21 EDT 2018 by benjamin
